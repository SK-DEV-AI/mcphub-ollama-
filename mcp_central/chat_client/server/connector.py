...
class ServerConnector:
    ...
    async def connect_to_servers(self, server_paths=None, server_urls=None, config_path=None, auto_discovery=False) -> Tuple[dict, list, dict]:
        ...
        if server_paths:
            ...
            for server in script_servers:
                if self.console: self.console.print(f"[cyan]Found server script: {server['path']}[/cyan]")
                else: logging.info(f"Found server script: {server['path']}")
        ...
        if server_urls:
            ...
            for server in url_servers:
                if self.console: self.console.print(f"[cyan]Found server URL: {server['url']} (type: {server['type']})[/cyan]")
                else: logging.info(f"Found server URL: {server['url']} (type: {server['type']})")
        ...
        if config_path:
            try:
                ...
                for server in config_servers:
                    if self.console: self.console.print(f"[cyan]Found server in config: {server['name']}[/cyan]")
                    else: logging.info(f"Found server in config: {server['name']}")
                all_servers.extend(config_servers)
            except Exception as e:
                if self.console: self.console.print(f"[red]Error loading server configurations: {str(e)}[/red]")
                else: logging.error(f"Error loading server configurations: {e}")

        # Auto-discover servers if enabled
        if auto_discovery:
            discovered_servers = auto_discover_servers()
            for server in discovered_servers:
                if self.console: self.console.print(f"[cyan]Auto-discovered server: {server['name']}[/cyan]")
                else: logging.info(f"Auto-discovered server: {server['name']}")
            all_servers.extend(discovered_servers)

        if not all_servers:
            message = "No servers specified or all servers were invalid. The client will continue without tool support."
            if self.console: self.console.print(Panel(f"[yellow]{message}[/yellow]", title="Warning", border_style="yellow", expand=False))
        ...
        if skipped_servers:
            if self.console:
                self.console.print(f"[red]Skipping servers due to connectivity issues: {', '.join(skipped_servers)}[/red]")
                self.console.print("[yellow]Servers must support HTTP or HTTPS protocols.[/yellow]")
            else:
                logging.warning(f"Skipping servers due to connectivity issues: {', '.join(skipped_servers)}")
                logging.warning("Servers must support HTTP or HTTPS protocols.")
        ...
        if not self.sessions:
            message = "Could not connect to any MCP servers! Check that server paths exist and are accessible."
            if self.console: self.console.print(Panel(f"[bold red]{message}[/bold red]", title="Error", border_style="red", expand=False))
    ...
    async def _connect_to_server(self, server: Dict[str, Any]) -> bool:
        ...
        if self.console: self.console.print(f"[cyan]Connecting to server: {server_name}[/cyan]")
        else: logging.info(f"Connecting to server: {server_name}")
        ...
        try:
            ...
            if server_type == "sse":
                ...
                if not url:
                    if self.console: self.console.print(f"[red]Error: SSE server {server_name} missing URL[/red]")
                    else: logging.error(f"Error: SSE server {server_name} missing URL")
                    return False
                ...
                url = self._get_url_from_server(server)
                if not url:
                    if self.console: self.console.print(f"[red]Error: HTTP server {server_name} missing URL[/red]")
                    else: logging.error(f"Error: HTTP server {server_name} missing URL")
                    return False
            ...
            if self.console: self.console.print(f"[green]Successfully connected to {server_name} with {len(server_tools)} tools[/green]")
            else: logging.info(f"Successfully connected to {server_name} with {len(server_tools)} tools")
            return True

        except FileNotFoundError as e:
            if self.console: self.console.print(f"[red]Error connecting to {server_name}: File not found - {str(e)}[/red]")
            else: logging.error(f"Error connecting to {server_name}: File not found - {e}")
            return False
        except PermissionError:
            if self.console: self.console.print(f"[red]Error connecting to {server_name}: Permission denied[/red]")
            else: logging.error(f"Error connecting to {server_name}: Permission denied")
            return False
        except Exception as e:
            if self.console: self.console.print(f"[red]Error connecting to {server_name}: {str(e)}[/red]")
            else: logging.error(f"Error connecting to {server_name}: {e}")
            return False
